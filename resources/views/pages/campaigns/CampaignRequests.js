...
export default class CampaignRequests extends Component {
    ...
    getMyCampaignRequests = () => {
        ...
        axios.get(config.api.userRequests,data).then( (res) => {
    ...
    ChangeStatus = (cid,uid,status) => () => {
        ...
        axios.post(config.api.userRequests,formdata,token).then( (res) => {
                                    ...
                                    { 
                                        this.state.showRequests.length!==0?(
                                            ...
                                            (request,id) =>
                                                ...
                                                <tr key={id}>
                                                    <td title="View User"><Link to={"/user/profile/"+request.user_id} target="_blank"><img className="img-round" src={request.user_image}></img>{request.user_name}</Link></td>
                                                    <td title="View Campaign"><Link to={"/campaign/dashboard/"+request.campaign_id} target="_blank"><img className="img-round" src={request.campaign_image}></img>{request.campaign_name}</Link></td>
